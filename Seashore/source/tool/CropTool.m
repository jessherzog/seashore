...
- (void)mouseDownAt:(IntPoint)where withEvent:(NSEvent *)event
{
	...
	movingCrop = NO;
	startPoint = where;
	if (IntPointInRect(where, cropRect)) {
		if (modifier != kControlModifier) {
			startOrigin = cropRect.origin;
			movingCrop = YES;
			return;
		}
	}
...
- (void)mouseDraggedTo:(IntPoint)where withEvent:(NSEvent *)event
{
	...
	if (movingCrop) {
		cropRect.origin.x = startOrigin.x + (where.x - startPoint.x);
		cropRect.origin.y = startOrigin.y + (where.y - startPoint.y);
	}
	else if (aspectType == kNoAspectType || aspectType == kRatioAspectType || oneToOne) {
